...
const RideRoutePolyline = memo(
  function RideRoutePolyline({ trackPoints, rideStatus }) {
    const map = useMap();
    const layersRef = useRef([]);

    // Memoize colors array
    const colors = useMemo(
      () => ["#00f032", "#3b82f6", "#f59e0b", "#ec4899", "#a78bfa", "#fb923c"],
      []
    );
    ...
    useEffect(() => {
      ...
      Object.entries(byUser).forEach(([, points]) => {
        ...
        const sorted = [...points].sort(
          (a, b) =>
            new Date(a.recorded_at || a.created_date) -
            new Date(b.recorded_at || b.created_date)
        );
    ...
    }, [trackPoints, map, rideStatus, colors]);

    return null;
  },
  (prevProps, nextProps) => {
    // Only re-render if track points significantly change or ride status changes
    return (
...
RideRoutePolyline.displayName = "RideRoutePolyline";
