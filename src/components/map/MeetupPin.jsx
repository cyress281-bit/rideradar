...
function createMeetupIcon(checkedIn, total, isStartingSoon) {
  ...
  return L.divIcon({
    ...
    html: `
      <div style="position:relative;width:44px;height:44px" role="img" aria-label="Meetup location marker">
        ...
        <div style="width:44px;height:44px;background:${bgColor};border-radius:50%;display:flex;align-items:center;justify-content:center;border:3px solid ${borderColor};box-shadow:0 0 18px ${glowColor};position:relative;flex-direction:column" aria-hidden="true">
          ...
          ${total > 0 ? `<div style="position:absolute;top:-6px;right:-6px;background:rgb(0,240,50);color:#000;border-radius:999px;font-size:11px;font-weight:800;padding:1px 5px;border:2px solid #000;min-width:16px;text-align:center" aria-label="${checkedIn} of ${total} riders checked in">${checkedIn}/${total}</div>` : ""}
        </div>
      </div>`,
    iconSize: [44, 44],
    iconAnchor: [22, 22],
