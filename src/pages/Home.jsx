  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="px-5 py-3">
        {/* Active Now — combined riding + meetup */}
        <div className="flex items-center gap-1.5 mb-2">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75" />
            <span className="relative inline-flex rounded-full h-2 w-2 bg-primary" />
          </span>
          <h2 className="text-xs font-semibold tracking-tight">Active Now</h2>
          {allVisibleRides.length > 0 && (
            <span className="text-[10px] text-muted-foreground ml-auto">{allVisibleRides.length} rides</span>
          )}
        </div>
